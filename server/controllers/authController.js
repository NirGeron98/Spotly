...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  // 4) Check if user changed password after the joken was issued
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  try {
    // Create email content with both text and HTML formats
    const { first_name } = user;
    const emailContent = createPasswordResetEmail(first_name, resetURL);

    // Send the email
    await sendEmail({
      email: user.email,
      subject: "איפוס סיסמה (בתוקף ל-10 דקות)",
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  if (!user) return next(new AppError("Wrong password. Try again!", 404));
