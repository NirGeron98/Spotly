...
exports.restoreSchedule = async (spotId, bookedScheduleId, options = {}) => {
  const { session, currentTime } = options;
  ...
  // If canceling mid-use, adjust the start time to current time
  if (currentTime && currentTime > scheduleToRestore.start_datetime) {
    // Create a new availability slot from current time to end time
    spot.availability_schedule.push({
      _id: new mongoose.Types.ObjectId(),
      start_datetime: currentTime,
      end_datetime: scheduleToRestore.end_datetime,
      is_available: true,
      type: scheduleToRestore.type,
      booking_id: null,
    });

    // Remove the original schedule
    spot.availability_schedule.splice(scheduleIndex, 1);
  } else {
    // Regular cancellation - just mark as available
